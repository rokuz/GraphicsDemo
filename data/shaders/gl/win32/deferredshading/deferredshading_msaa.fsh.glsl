...
uniform sampler2DMS dataBlockMap1;
uniform usampler2DMS dataBlockMap2;
...
void main()
{
	...
	{
		uvec4 dataBlock2 = texelFetch(dataBlockMap2, coords, i);
		sumMatId += (dataBlock2.x & 0x0000ffff);
	}
	if (sumMatId < 1) discard;
	...
	{
		uvec4 dataBlock2 = texelFetch(dataBlockMap2, coords, s);
		uint coverage = (dataBlock2.x >> 16);
		
		if (coverage & (1 << gl_SampleID))
		{
			vec4 dataBlock1 = texelFetch(dataBlockMap1, coords, s);
			...
			result += vec4(clamp(ambient + diffuse + specular, 0.0f, 1.0f), diffTex.a);
