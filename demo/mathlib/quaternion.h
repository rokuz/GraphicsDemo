...
public:
	...
	vector3 z_direction() const
	{
		return vector3(2 * w * y + 2 * x * z,
					   2 * y * z - 2 * x * w,
					   w * w + z * z - x * x - y * y);
	}

	vector3 x_direction() const
	{
        return vector3(w * w + x * x - y * y - z * z,
                       2 * w * z + 2 * y * x,
                       2 * x * z - 2 * y * w);
	}
	
	vector3 y_direction() const
	{
        return vector3(2 * y * x - 2 * z * w,
                       w * w + y * y - z * z - x * x,
                       2 * z * y + 2 * x * w);
	}
